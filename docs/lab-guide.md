...
```
                                 ...
                                 └─────────────┬──────────────┘
                                               ...
                                               ▼
                          ┌──────────────────────────────────────┐
                          │   Stage 2: SecurityScanAndBuild      │
                          │   (CodeBuild + buildspec.yml)        │
                          │                                      │
                          │   1) trivy config  Dockerfile        │
                          │   2) trivy fs --scanners secret      │
                          │   3) docker build                    │
                          │   4) trivy image  <imagen>           │
                          │                                      │
                          │   ¿HIGH o CRITICAL? ──► FAIL ✗       │
                          │                                      │
                          │   Si limpio  ──►  docker push        │
                          └─────────────────┬────────────────────┘
                                            │
                                            ▼
                          ┌──────────────────────────────────────┐
                          │              Amazon ECR              │
                          │  (scan-on-push con Inspector v2)     │
                          └──────────────────────────────────────┘
```
...
```bash
# Instalación rápida de Trivy local (Linux/Mac)
...
```bash
git clone https://github.com/<tu-usuario>/devsecops-workshop.git
cd devsecops-workshop

# Copiar el contenido de este workshop dentro del repo
# (app/, vulnerable/, secure/, infrastructure/, trivy/, buildspec.yml)

git add .
git commit -m "init: workshop devsecops"
...
aws codestar-connections create-connection \
    ...
    --connection-name devsecops-workshop-conn
```
...
aws cloudformation deploy \
    --stack-name devsecops-ecr \
    --template-file infrastructure/01-ecr.yaml \
...
```bash
aws ecr describe-repositories --repository-names devsecops-demo
```
...
aws cloudformation deploy \
    ...
    --parameter-overrides \
        ...
        CodeStarConnectionArn=<arn-del-paso-1>
```
...
```bash
aws ecr list-images --repository-name devsecops-demo
...
aws ecr delete-repository --repository-name devsecops-demo --force
aws cloudformation delete-stack --stack-name devsecops-ecr
aws codestar-connections delete-connection --connection-arn <arn>
```
